  ...
  return (
    <div style={{ paddingTop: 62 }}>
        ...
        <div style={{ position: "relative", zIndex: 1, minHeight: "100vh", display: "grid", gridTemplateColumns: "1fr 1fr", alignItems: "center", maxWidth: 1280, margin: "0 auto", padding: "40px 48px 0 64px" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", paddingRight: 48 }}>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", animation: "fadeUp 0.55s 0.24s cubic-bezier(0.22,1,0.36,1) both" }}>
              <button onClick={() => setPage("systems")} style={{ background: "var(--white)", color: "var(--black)", border: "none", padding: "13px 28px", fontSize: 14, fontWeight: 700, borderRadius: 8, letterSpacing: "-0.3px" }}>Learn More →</button>
              <button onClick={() => setPage("pricing")} style={{ background: "transparent", color: "#888", border: "1px solid rgba(255,255,255,0.12)", padding: "13px 28px", fontSize: 14, fontWeight: 600, borderRadius: 8, letterSpacing: "-0.3px" }}>Pricing</button>
            </div>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "100px 48px" }}>
        ...
        <h2 style={{ fontSize: "clamp(32px, 3.5vw, 52px)", fontWeight: 800, letterSpacing: "-2px", lineHeight: 1.05, color: "var(--white)", marginBottom: 48 }}>Tools that get things done.</h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
          <button
            onClick={() => setPage("paychaser")}
            onMouseMove={e => handleCardMove("feat", e)}
            style={{ gridColumn: "1 / -1", border: "1px solid var(--border)", borderRadius: 12, padding: 36, display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative", overflow: "hidden", background: "#0c0c0c", cursor: "pointer", textAlign: "left", transition: "border-color 0.2s, transform 0.2s" }}>
            <div>
...
const SystemsPage = ({ setPage }) => (
  <div style={{ paddingTop: 62 }}>
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "140px 64px 72px", animation: "fadeUp 0.55s cubic-bezier(0.22,1,0.36,1) both" }}>
      ...
      <h1 style={{ fontSize: "clamp(40px, 5vw, 72px)", fontWeight: 800, letterSpacing: "-2.5px", lineHeight: 1.05, color: "var(--white)", marginBottom: 20 }}>Tools that get things done.</h1>
...
════════════════════════════════════════════ */
const STEPS = [
  { num: "Step 01 — Connect", heading: "Add your invoices in seconds.", body: <>Connect your existing tools or add invoices manually. Almondy works with your workflow — not against it. <strong style={{ color: "#888" }}>No complex setup, no training required.</strong> Just drop in your invoices and you're live.</>, visLabel: "Invoice Import", vis: <VisInvoiceImport /> },
  { num: "Step 02 — Automate", heading: "Set your reminder rules. Once.", body: <>Tell Almondy how you want to chase — friendly nudge on day 3, firmer tone on day 7, final notice on day 14. <strong style={{ color: "#888" }}>It handles the timing, the wording, and the sending.</strong> You never have to think about it again.</>, visLabel: "Reminder Schedule", vis: <VisSchedule /> },
  { num: "Step 03 — Track", heading: "Know exactly where every invoice stands.", body: <>Your dashboard shows every invoice, every reminder sent, and every payment received — in real time. <strong style={{ color: "#888" }}>No more chasing spreadsheets.</strong> Just a clear, honest view of what's paid and what's not.</>, visLabel: "Live Dashboard", vis: <VisTrack /> },
  { num: "Step 04 — Collect", heading: "Get paid. Move on.", body: <>When a client pays, Almondy marks it done and stops all reminders automatically. <strong style={{ color: "#888" }}>No double-checking, No Worry.</strong> Automated end to end so you can stop worrying and get back to work.</>, visLabel: "Payment Received", vis: <VisCelebrate /> }
];
    ...
    <div style={{ paddingTop: 62 }}>
      <div style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 900, margin: "0 auto", padding: "96px 48px 72px", textAlign: "center", animation: "fadeUp 0.6s cubic-bezier(0.22,1,0.36,1) both" }}>
          ...
          <h1 style={{ fontSize: "clamp(42px, 6vw, 80px)", fontWeight: 800, letterSpacing: "-3px", lineHeight: 1.02, color: "var(--white)", marginBottom: 22 }}>Chase money.<br />Not clients.</h1>
...
const PlanCard = ({ name, price, period, desc, cta, ctaStyle, features, featured = false, badge = null }) => {
  const [pos, setPos] = useState({ x: 50, y: 50 });
  return (
    <div onMouseMove={e => { const r = e.currentTarget.getBoundingClientRect(); setPos({ x: ((e.clientX - r.left) / r.width * 100).toFixed(1), y: ((e.clientY - r.top) / r.height * 100).toFixed(1) }); }}
      style={{ position: "relative", border: `1px solid ${featured ? "rgba(255,255,255,0.15)" : "var(--border)"}`, borderRadius: 16, padding: "32px 28px 28px", display: "flex", flexDirection: "column", background: featured ? "#0f0f0f" : "#0c0c0c", transform: featured ? "scale(1.03)" : "none", boxShadow: featured ? "0 0 0 1px rgba(255,255,255,0.06), 0 32px 64px rgba(0,0,0,0.5)" : "none", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        <button style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "100%", padding: "13px 24px", fontSize: 13.5, fontWeight: 700, borderRadius: 9, border: ctaStyle === "solid" ? "none" : "1px solid rgba(255,255,255,0.1)", background: ctaStyle === "solid" ? "var(--white)" : "transparent", color: ctaStyle === "solid" ? "var(--black)" : "#666", marginBottom: 28, letterSpacing: "-0.3px" }}>{cta}</button>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
          {features.map(([on, label]) => (
            <div key={label} style={{ display: "flex", alignItems: "flex-start", gap: 10, fontSize: 13, lineHeight: 1.5, color: on ? "#8a8a8a" : "#383838" }}>
              <div style={{ width: 16, height: 16, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1, fontSize: 8, fontWeight: 900, background: on ? "rgba(34,197,94,0.1)" : "rgba(255,255,255,0.03)", color: on ? "var(--green)" : "#2e2e2e", border: `1px solid ${on ? "rgba(34,197,94,0.18)" : "rgba(255,255,255,0.06)"}` }}>{on ? "✓" : "—"}</div>
...
const PRICES = {
  monthly: { basic: "0", pro: "29.99", max: "59.99", basicP: "Free forever", proP: "AUD per month", maxP: "AUD per month" },
  annual:  { basic: "0", pro: "23.99", max: "47.99", basicP: "Free forever", proP: "AUD per month, billed annually", maxP: "AUD per month, billed annually" }
};
...
const FAQS = [
  ...
  ["Is there really a free trial on Pro?", "Yes — 3 days, full Pro features, card required. If it's not for you, cancel before the trial ends and you won't be charged."],
    ...
    <div style={{ paddingTop: 62 }}>
      <div style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, textAlign: "center", padding: "88px 48px 64px", animation: "fadeUp 0.55s cubic-bezier(0.22,1,0.36,1) both" }}>
          ...
          <h1 style={{ fontSize: "clamp(38px, 5.5vw, 72px)", fontWeight: 800, letterSpacing: "-2.5px", lineHeight: 1.03, color: "var(--white)", marginBottom: 18 }}>Simple pricing.<br />No surprises.</h1>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "0 48px 80px" }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
          <thead>
            <tr>{["Feature","Basic","Pro","Max"].map((h,i) => <th key={h} style={{ padding: "15px 20px", textAlign: i === 0 ? "left" : "center", fontSize: 11.5, fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "var(--muted)", background: "#0a0a0a", borderBottom: "1px solid var(--border)", fontFamily: "var(--mono)", width: i === 0 ? "40%" : undefined }}>{h}</th>)}</tr>
          </thead>
          <tbody>
            {[["Active invoices","5","Unlimited","Unlimited"],["Email reminders","✓","✓","✓"],["SMS reminders","—","✓","✓"],["Automated sequences","—","✓","✓"],["Smart escalation","—","✓","✓"],["White-label branding","—","—","✓"],["API access","—","—","✓"],["Support","Community","Priority email","Dedicated manager"]].map(([feat,...vals]) => (
              <tr key={feat} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                {vals.map((v,i) => <td key={i} style={{ padding: "13px 20px", textAlign: "center", fontSize: v === "✓" ? 15 : v === "—" ? 15 : 12.5, color: v === "✓" ? "var(--green)" : v === "—" ? "#222" : "#777", fontWeight: v !== "✓" && v !== "—" ? 600 : undefined }}>{v}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div style={{ maxWidth: 660, margin: "0 auto", padding: "0 48px 100px" }}>
        <h2 style={{ fontSize: "clamp(28px, 3vw, 44px)", fontWeight: 800, letterSpacing: "-1.5px", color: "var(--white)", textAlign: "center", marginBottom: 44 }}>Questions? Answered.</h2>
        {FAQS.map(([q, a], i) => (
          ...
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div>
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSend()} placeholder="you@yourbusiness.com" autoFocus
                  style={{ width: "100%", padding: "13px 16px", background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, fontSize: 15, color: "var(--white)", outline: "none", fontFamily: "var(--font)" }} />
              </div>
              <button onClick={handleSend} disabled={!email.includes("@") || loading}
            ...
            <p style={{ textAlign: "center", fontSize: 12, color: "#444", marginTop: 20, lineHeight: 1.6 }}>
              By continuing you agree to our <span style={{ color: "#666", cursor: "pointer" }}>Terms</span> and <span style={{ color: "#666", cursor: "pointer" }}>Privacy Policy</span>.
            </p>
          </>
        ) : (
          <>
            ...
            <div style={{ background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 12, padding: "20px", marginBottom: 20, display: "flex", alignItems: "flex-start", gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "#444", lineHeight: 1.7 }}>Click the link to sign in. It expires in 15 minutes. If you don't see it, check your spam folder.</div>
              </div>
            </div>
            <button onClick={() => setStep("email")} style={{ width: "100%", padding: "12px", background: "transparent", color: "#555", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, fontSize: 13, fontWeight: 600 }}>← Use a different email</button>
          </>
        )}
      </div>
    </div>
...
const OnboardingPage = ({ setPage, user, setUser }) => {
  ...
  const handleContinue = async () => {
    ...
    setLoading(true);
    if (user?.id) {
      await supabase.from("profiles").update({ biz_name: bizName.trim() }).eq("id", user.id);
    }
    setUser(u => ({ ...u, bizName: bizName.trim() }));
    setPage("paywall");
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--black)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, position: "relative", overflow: "hidden" }}>
      ...
      <button onClick={() => setPage("home")} style={{ position: "absolute", top: 28, left: 32, display: "flex", alignItems: "center", background: "none", border: "none" }}>
        <AlmondyLogo height={20} />
      </button>
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 480, animation: "fadeUp 0.5s cubic-bezier(0.22,1,0.36,1) both" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <input type="text" value={bizName} onChange={e => setBizName(e.target.value)} onKeyDown={e => e.key === "Enter" && handleContinue()} placeholder="e.g. Smith Electrical" autoFocus
          ...
          <button onClick={handleContinue} disabled={!bizName.trim() || loading}
...
const AppNav = ({ user, setPage }) => (
  <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, height: 58, background: "rgba(8,8,8,0.95)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <AlmondyLogo height={22} />
      {user?.plan === "free" && (
        <div style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 999, padding: "2px 10px", fontSize: 10.5, fontWeight: 700, color: "#f59e0b", letterSpacing: "0.5px" }}>FREE PLAN</div>
      )}
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      {user?.plan === "free" && (
        <button onClick={() => setPage("paywall")} style={{ padding: "7px 16px", background: "var(--green)", color: "var(--black)", border: "none", borderRadius: 7, fontSize: 12.5, fontWeight: 700, letterSpacing: "-0.2px" }}>Upgrade to Pro</button>
      )}
      ...
      <div style={{ display: "flex", flex: 1, paddingTop: 58 }}>
        ...
        <div style={{ flex: 1, padding: "32px 36px", overflowY: "auto" }}>
          {user?.plan === "free" && showUpgradeBanner && (
            <div style={{ background: "rgba(245,158,11,0.07)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 12, padding: "14px 20px", display: "flex", alignItems: "center", gap: 16, marginBottom: 28 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
            <div>
              <h1 style={{ fontSize: 22, fontWeight: 800, letterSpacing: "-0.8px", color: "var(--white)", marginBottom: 4 }}>{user?.bizName ?? "My Dashboard"}</h1>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 28 }}>
            {[{ label: "Collected", value: `$${collected.toLocaleString()}`, sub: "This month", green: true }, { label: "Outstanding", value: `$${outstanding.toLocaleString()}`, sub: `${MOCK_INVOICES.filter(i => i.status !== "paid").length} invoices pending`, green: false }, { label: "Overdue", value: `$${overdue.toLocaleString()}`, sub: `${MOCK_INVOICES.filter(i => i.status === "overdue").length} clients flagged`, green: false }].map(({ label, value, sub, green }) => (
          ...
          <div style={{ background: "#0a0a0a", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 120px 120px 100px 44px", gap: 0, padding: "12px 20px", borderBottom: "1px solid rgba(255,255,255,0.05)", background: "#080808" }}>
              {["Client", "Invoice", "Amount", "Status", ""].map(h => <div key={h} style={{ fontSize: 10.5, fontWeight: 700, color: "#333", letterSpacing: "1.2px", textTransform: "uppercase", fontFamily: "var(--mono)" }}>{h}</div>)}
            </div>
            {MOCK_INVOICES.map((inv, i) => {
              ...
              return (
                <div key={inv.id} style={{ display: "grid", gridTemplateColumns: "1fr 120px 120px 100px 44px", gap: 0, padding: "16px 20px", borderBottom: i < MOCK_INVOICES.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none", alignItems: "center", transition: "background 0.15s" }}
                  ...
                  <button onClick={() => setPage("paywall")} style={{ background: "none", border: "none", color: "#2a2a2a", fontSize: 16, cursor: "pointer", padding: 4 }}>···</button>
                </div>
              );
            })}
            {user?.plan === "free" && (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
            {[{ icon: "🔔", title: "Automated Reminders", desc: "Set it and forget it. PayChaser chases for you.", locked: user?.plan === "free" }, { icon: "📊", title: "Analytics & Reports", desc: "See your collection rate, avg days to pay, and more.", locked: user?.plan === "free" }].map(({ icon, title, desc, locked }) => (
              <div key={title} onClick={() => locked && setPage("paywall")} style={{ background: "#0a0a0a", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "20px", position: "relative", overflow: "hidden", cursor: locked ? "pointer" : "default" }}>
...
const PaywallPage = ({ setPage, user, setUser }) => {
  ...
  const handleUpgrade = async (planId) => {
    ...
    try {
      ...
      if (error) { console.error(error); setLoadingPlan(null); }
    } catch (err) {
      console.error(err);
      setLoadingPlan(null);
    }
  ...
  const PLAN_DATA = [
    { id: "pro", name: "Pro", badge: "⚡ Most Popular", desc: "For growing businesses that need automated chasing without the awkward follow-ups.", features: ["Unlimited invoices","Automated reminder sequences","Email + SMS reminders","Smart escalation logic","Advanced analytics","Priority email support"], featured: true },
    { id: "max", name: "Max", badge: "Enterprise", desc: "For agencies managing invoices at scale with full white-label and team control.", features: ["Everything in Pro","White-label branding","Custom reminder templates","Team member access","API access","Dedicated account manager"], featured: false },
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--black)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 24px", position: "relative", overflow: "hidden" }}>
      ...
      <button onClick={() => setPage("dashboard")} style={{ position: "absolute", top: 24, left: 28, display: "flex", alignItems: "center", gap: 7, background: "none", border: "none", color: "#555", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>← Back to dashboard</button>
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 860, animation: "fadeUp 0.5s cubic-bezier(0.22,1,0.36,1) both" }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          ...
          <h1 style={{ fontSize: "clamp(26px, 4vw, 36px)", fontWeight: 800, letterSpacing: "-1.5px", color: "var(--white)", marginBottom: 10 }}>You've hit the free limit</h1>
          <p style={{ fontSize: 14, color: "#666", lineHeight: 1.7, maxWidth: 440, margin: "0 auto 28px" }}>You're on 5 of 5 free invoices. Upgrade to keep chasing — and let Almondy do the hard work.</p>
          <div style={{ display: "inline-flex", background: "#0f0f0f", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 999, padding: "3px 4px" }}>
            {["monthly", "annual"].map(m => (
              <button key={m} onClick={() => setBilling(m)} style={{ padding: "7px 20px", borderRadius: 999, border: "none", fontSize: 13, fontWeight: 600, background: billing === m ? "var(--white)" : "transparent", color: billing === m ? "var(--black)" : "#555", display: "inline-flex", alignItems: "center", gap: 8, transition: "all 0.2s", cursor: "pointer", fontFamily: "var(--font)" }}>
                {m.charAt(0).toUpperCase() + m.slice(1)}
                {m === "annual" && <span style={{ fontSize: 9.5, fontWeight: 700, color: "var(--green)", background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 999, padding: "1px 7px" }}>Save 20%</span>}
              </button>
            ))}
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 28 }}>
          {PLAN_DATA.map(plan => {
            ...
            return (
              <div key={plan.id} style={{ position: "relative", background: plan.featured ? "#0f0f0f" : "#0c0c0c", border: `1px solid ${plan.featured ? "rgba(255,255,255,0.14)" : "rgba(255,255,255,0.07)"}`, borderRadius: 18, padding: "28px 26px 26px", display: "flex", flexDirection: "column", boxShadow: plan.featured ? "0 0 0 1px rgba(255,255,255,0.04), 0 24px 60px rgba(0,0,0,0.5)" : "none", overflow: "hidden" }}>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 10, fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: plan.featured ? "var(--green)" : "#555", background: plan.featured ? "rgba(34,197,94,0.1)" : "rgba(255,255,255,0.05)", border: `1px solid ${plan.featured ? "rgba(34,197,94,0.22)" : "rgba(255,255,255,0.09)"}`, borderRadius: 999, padding: "4px 11px", marginBottom: 18, width: "fit-content" }}>{plan.badge}</div>
                ...
                <button onClick={() => handleUpgrade(plan.id)} disabled={!!loadingPlan}
                  style={{ width: "100%", padding: "13px 20px", background: plan.id === "pro" ? "var(--white)" : "rgba(255,255,255,0.08)", color: plan.id === "pro" ? "var(--black)" : "var(--white)", border: plan.id === "pro" ? "none" : "1px solid rgba(255,255,255,0.18)", borderRadius: 10, fontSize: 14, fontWeight: 700, letterSpacing: "-0.3px", marginBottom: 22, cursor: loadingPlan ? "not-allowed" : "pointer", opacity: loadingPlan && !isLoading ? 0.4 : 1, transition: "all 0.2s", fontFamily: "var(--font)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 28 }}>
          {["No lock-in contracts", "Cancel any time", "Instant access"].map(t => (
...
/* ════════════════════════════════════════════
   ROOT APP — Supabase auth wired up
════════════════════════════════════════════ */
export default function App() {
  ...
  useEffect(() => {
    const handleSession = async (session) => {
      ...
      if (!profile) {
        ...
        if (!profile.biz_name) setPage("onboarding");
        else if (!hasActivePlan) setPage("paywall");
  ...
  return (
    <>
      ...
      {page === "auth"       && <AuthPage       setPage={handleSetPage} />}
